...
def test_download_and_process(mocker, caplog):
    ...
    with caplog.at_level(logging.DEBUG):
        download_and_process([], "moti", "postgresql://user:password@db3.pcic/somdb", [])
    for record in caplog.records:
        assert "password" not in record.message

@pytest.mark.parametrize(("resolution", "direction", "expected"), (
...
@pytest.mark.parametrize(("resolution", "direction"), (
    ("hour", "up"),
    ("hour", "down"),
    ("day", "up"),
    ("day", "down"),
