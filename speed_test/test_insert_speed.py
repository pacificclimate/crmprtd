...
def generate_worst_obs(sesh, num_obs):
    ...
    dup_ob = Obs(id=ob.id, time=ob.time, history_id=ob.history_id,
                 vars_id=ob.vars_id, datum=ob.datum)
...
def results_to_file(results, name, num_obs, case, num_iter,
                    chunk_size, num_bad):
...
def run_test(num_obs, num_iter, bad_obs, chunk_size, setup_list, test_list):
    for test in test_list:
        for setup in setup_list:
            ...
            results_to_file(results, get_name(test), num_obs, get_case(setup),
                            num_iter, chunk_size, bad_obs)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-t', '--iterations',
                        default=10,
                        help='Number of times the testing code will be run')
    parser.add_argument('-o', '--observations',
                        default=30000,
                        help='Number of observations to be inserted')
    parser.add_argument('-b', '--bad_observations',
                        default=4000,
                        help='Number of bad observations to include')
    parser.add_argument('-c', '--chunk_size',
                        default=4096,
                        help='Size of the chunks for the chunk insertion '
                             'strategy')
    args = parser.parse_args()

    setup_list = get_setup_code(args.connection_string, int(args.observations),
                                int(args.bad_observations))
    test_list = get_test_code(int(args.chunk_size))

    run_test(int(args.observations), int(args.iterations),
             int(args.bad_observations), int(args.chunk_size), setup_list,
             test_list)
