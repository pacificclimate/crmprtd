...
for net in ${networks}; do
    ...
    echo "download_${net} -L logging.yaml --log_filename ${log_file} -d \"${TIME}\" | tee ${cache_file} | crmprtd_process -c postgresql://crmprtd@monsoon/crmp -N ${net} -L logging.yaml --log_filename ${log_file}" | batch
done
...
for net in ${networks}; do
    export log_file=~/${net}/logs/${net}_json.log
    export cache_file=~/${net}/cache/${net}_$(date +'%Y-%m-%dT%H:%M:%S').xml
    echo "download_${net} -L ~/logging.yaml --log_filename ${log_file} -d \"${TIME}\" | tee ${cache_file} | crmprtd_process -c postgresql://crmprtd@dbnorth/metnorth -N ${net} -L ~/logging.yaml --log_filename ${log_file}" | batch
