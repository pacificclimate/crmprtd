...
# Not intended as anything more than a convenience function when performing
# a manual recovery on EC data
...
def main(opts, args):
    ...
    logdir = makeDir(os.path.join(opts.logdir, 'EC_' +
    ...
    with open(os.path.join(logdir, 'recovery.txt'), 'w') as f:
        for prov in ['BC', 'YT']:
            for freq in ['hourly', 'daily']:
                s = ("python {script} -e bveerman@uvic.ca -c '{conn_string}' "
                     "-p {prov} -F {freq} --starttime '{s}' --endtime '{e}' "
                     "--threshold 1000 -l {logdir}/ec_recovery_{prov}_{freq}"
                     "_{ss}_{ee}.log").format(script=script,
...
if __name__ == '__main__':
    parser = OptionParser()
    parser.add_option('-c', '--conn_string', dest='conn_string',
    ...
    parser.add_option('--logdir',
                      help=('Base directory in which to create a date '
                            'specific dir for log files.  Must have write '
                            'permissions'))
    parser.add_option('--reason',
                      help=('Tag the folder created with a reason for the '
                            'recovery'))
    parser.add_option('--prefix',
                      help=('Script location prefix if recovery script not in '
                            'PATH'))
    parser.add_option('--starttime', dest='stime',
                      help=("Start time of range to recover (interpreted with "
                            " strptime(format='%Y/%m/%d %H:%M:%S')"))
    parser.add_option('--endtime', dest='etime',
                      help=("End time of range to recover (interpreted with "
                            "strptime(format='%Y/%m/%d %H:%M:%S')"))
    parser.add_option('-T', '--threshold', dest='thresh',
                      help=('Distance threshold to use when matching '
                            ...
                            'they have the same id, name, and are within this '
                            'threshold'))
    parser.add_option('-D', '--diag', dest='diag', action="store_true",
