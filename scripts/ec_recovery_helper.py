...
def main(opts, args):
    ...
    logdir = makeDir(
        os.path.join(opts.logdir, "EC_" + stime.strftime("%Y%m%d") + "_" + opts.reason)
    )
    ...
    with open(os.path.join(logdir, "recovery.txt"), "w") as f:
        for prov in ["BC", "YT"]:
            for freq in ["hourly", "daily"]:
                s = (
                    "python {script} -e bveerman@uvic.ca -c '{conn_string}' "
...
if __name__ == "__main__":
    parser = OptionParser()
    parser.add_option(
        "-c", "--conn_string", dest="conn_string", help="PostgreSQL connection string"
    )
    parser.add_option(
        "-e",
        "--error_email",
    ...
    parser.add_option(
        "--logdir",
        help=(
            "Base directory in which to create a date "
            "specific dir for log files.  Must have write "
            "permissions"
        ),
    )
    parser.add_option(
        "--reason", help=("Tag the folder created with a reason for the " "recovery")
    )
    parser.add_option(
        "--prefix", help=("Script location prefix if recovery script not in " "PATH")
    )
    parser.add_option(
        "--starttime",
        dest="stime",
    ...
    parser.add_option(
        ...
        dest="thresh",
        help=(
            "Distance threshold to use when matching "
    ...
    parser.set_defaults(
        conn_string="dbname=crmp user=crmprtd",
