...
def main(args):
    ...
    if args.file_pattern:
        log.info("Using file pattern", extra={"file_pattern": args.file_patter})
    ...
    for fname in f_list:
        ...
        try:
            ...
            sesh.rollback()
            log.critical(
                "Serious errors with MOTIe rtd, see logs", extra={"log": args.log}
            )
            continue
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-p", "--file_pattern", default=None, help="directory to process for xml files"
    )
    parser.add_argument(
        "-f", "--filename", default=None, help="MPO-XML file to process"
    )
    parser.add_argument(
        "-o",
        "--output_dir",
