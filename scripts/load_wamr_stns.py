...
if __name__ == '__main__':
    ...
    for row in rows:
        ...
        q = sesh.query(Station).filter(Station.network == env_aqn).filter(Station.native_id == stn.EMS_ID)
        if q.count():
            print("We already have station {}/{} in the database".format(stn.EMS_ID, stn.STATION))
            ...
            if len(estn.histories) == 1:
                ...
                tol = .001
                if abs(float(hist.lon) - stn.LONG) < tol and abs(float(hist.lat) - stn.LAT) < tol and hist.elevation == stn.ELEVATION:
                    print("Good, everything agrees")
                else:
                    print("Our single history entry for this station disagrees with the input and we need human disambiguation")
                    print("Us: {} {} {}, Them: {} {} {}. Pick one (type 'l' or 'r')".format(hist.lon, hist.lat, hist.elevation, stn.LONG, stn.LAT, stn.ELEVATION))
                    choice = input()
                    if choice == 'l':
                        ...
                        hist.the_geom = 'SRID=4326;POINT({} {})'.format(stn.LONG, stn.LAT)
                    else:
                        print("OK, I'll skip it")
            else:
                print("Multiple history entries for this station. No clue what to do.")
        else:
            print("Station {}/{} is *not* in the database. We'll insert.".format(stn.EMS_ID, stn.STATION))
            estn = Station(native_id=stn.EMS_ID, network=env_aqn)
            hist = History(station=estn, lat=stn.LAT, lon=stn.LONG,
                           the_geom='SRID=4326;POINT({} {})'.format(stn.LONG, stn.LAT),
                           elevation=stn.ELEVATION, province='BC',
                           freq='1-hourly', station_name=stn.STATION)
