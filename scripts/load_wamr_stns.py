...
if __name__ == "__main__":
    ...
    for row in rows:
        ...
        q = (
            sesh.query(Station)
            .filter(Station.network == env_aqn)
            .filter(Station.native_id == stn.EMS_ID)
        )
        if q.count():
            print(
                ("We already have station {}/{} in the " "database").format(
                    stn.EMS_ID, stn.STATION
                )
            )
            ...
            if len(estn.histories) == 1:
                ...
                tol = 0.001
                if (
                    abs(float(hist.lon) - stn.LONG) < tol
                    and abs(float(hist.lat) - stn.LAT) < tol
                    and hist.elevation == stn.ELEVATION
                ):
                    print("Good, everything agrees")
                else:
                    print(
                        "Our single history entry for this station "
                ...
                print(
                    "Multiple history entries for this station. " "No clue what to do."
                )
        else:
            print(
                ("Station {}/{} is *not* in the database. " "We'll insert.").format(
                    stn.EMS_ID, stn.STATION
                )
            )
