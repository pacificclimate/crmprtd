...
def main(opts, args):
    ...
    for file_ in files:
        ...
        try:
            rv = process(sesh, et)
            logging.info("Processed file", extra={"file": file_, "results": rv})
...
def copy_stations_to_sqlite(src_dsn, dest_dsn):
    ...
    for stn in q.all():
        ...
        new_obj = Station(
            native_id=stn.native_id, network_id=net.id, histories=histories
        )
...
if __name__ == "__main__":
    ...
    parser.add_option(
        ...
        "--cache_dir",
        dest="cache_dir",
        help=("directory from which to get the XML files to " "process"),
    )
    parser.add_option(
        "-S",
        "--source_dsn",
        dest="source_dsn",
        help=("optional DSN from which to get stations for " "pre-insert"),
    )
    parser.add_option(
        "-n",
        "--new_db",
    ...
    parser.set_defaults(
        connection_string="sqlite:////tmp/crmp.sqlite",
