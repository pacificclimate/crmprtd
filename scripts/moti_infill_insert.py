...
def copy_stations_to_sqlite(src_dsn, dest_dsn):
    ...
    q = src_sesh.query(Station).join(History).join(Network).filter(Network.name == 'MoTIe')
    for stn in q.all():
        histories = [ History(station_name=hist.station_name) for hist in stn.histories ]
        new_obj = Station(native_id=stn.native_id, network_id=net.id, histories=histories)
        dest_sesh.add(new_obj)
    dest_sesh.commit()

    q = src_sesh.query(Variable).join(Network).filter(Network.name == 'MoTIe')
    for var in q.all():
        v = Variable(name=var.name, standard_name=var.standard_name, cell_method=var.cell_method, network_id=net.id, unit=var.unit)
        dest_sesh.add(v)
    dest_sesh.commit()
...
if __name__ == '__main__':
    ...
    parser = OptionParser()
    parser.add_option('-c', '--connection_string', dest='connection_string', help='SQLALchemy DSN')
    ...
    parser.add_option('-C', '--cache_dir', dest='cache_dir', help='directory from which to get the XML files to process')
    parser.add_option('-S', '--source_dsn', dest='source_dsn', help='optional DSN from which to get stations for pre-insert')
    parser.add_option('-n', '--new_db', dest='new_db', help='create a fresh database at the connection_string?', action="store_true")
    ...
    parser.set_defaults(connection_string='sqlite:////tmp/crmp.sqlite',
                        ...
                        log='/home/data/projects/crmp/data/MoTI_Downloads/logs/',
                        error_email='hiebert@uvic.ca',
                        cache_dir='/home/data/projects/crmp/data/MoTI_Downloads/auto',
                        source_dsn=None,
                        new_db=False,
                        )
