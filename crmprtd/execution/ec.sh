...
for prov in ${provinces}; do
    ...
    echo "download_${net} -p ${lower_prov} -F ${frequency} -L ~/logging.yaml --log_filename ${log_file} -t \"${TIME}\" | tee ${cache_file} | crmprtd_process -c postgresql://crmprtd@monsoon/crmp -N ${net} -L ~/logging.yaml --log_filename ${log_file}" | batch
done
