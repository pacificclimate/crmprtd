...
def summarize_raw_station(csv_file, station_row, data_path):
    ...
    # df_data = pd.read_csv(os.path.join(data_path, csv_file), encoding="latin1", low_memory=False)
    
    df_data = safe_read_csv(os.path.join(data_path, csv_file))
    ...
    # df_data = df_data.applymap(fix_encoding)
